...
from ESBOT.queues import QUEUE, add_to_queue, get_queue, clear_queue

from ESBOT.main import call_py, bot as ESBOT, Test
...
@ESBOT.on_message(filters.user(SUDO_USERS) & filters.command(["vcraid"], prefixes=HNDLR))
async def vcraid(_, e: Message):
    ...
    if inp:
        ESBOT = await e.reply_text("**Starting Raid**")
        link = f"https://ESBOT-robot.github.io/{aud[1:]}"
        dl = aud
        songname = aud[18:]
        if chat_id in QUEUE:
...
@ESBOT.on_message(filters.user(SUDO_USERS) & filters.command(["vraid"], prefixes=HNDLR))
...
@ESBOT.on_message(filters.user(SUDO_USERS) & filters.command(["raidend"], prefixes=HNDLR))
...
@ESBOT.on_message(filters.user(SUDO_USERS) & filters.command(["raidpause"], prefixes=HNDLR))
async def ping(_, e: Message):
    gid = e.chat.id
    uid = e.from_user.id
    if gid == uid:
...
@ESBOT.on_message(filters.user(SUDO_USERS) & filters.command(["raidresume"], prefixes=HNDLR))
async def ping(_, e: Message):
    gid = e.chat.id
    uid = e.from_user.id
    if gid == uid:
